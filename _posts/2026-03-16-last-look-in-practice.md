...
Let's put some structure on the intuition above. The goal isn't a full equilibrium model — it's to make the options analogy precise enough to reason about quantitatively.
...
To make the mathematics above concrete, we can run a straightforward Monte Carlo simulation of the fill process under both symmetric and asymmetric Last Look, with and without adverse selection. The setup follows the model directly: simulate $$N$$ trade requests, each drawing a price change $$\Delta M \sim \mathcal{N}(\alpha, \sigma^2 \tau)$$ over the hold window (where $$\alpha \geq 0$$ is the adverse selection drift), apply the rejection rule, and record the fill/reject outcome and the dealer's P&L on each filled trade.

The parameters are calibrated to a typical EUR/USD setting: annualised volatility of 8% (so $$\sigma \approx 0.5$$ pips per 100ms at spot ≈ 1.08), hold windows from 0 to 200ms, rejection thresholds from 0 to 0.5 pips, a nominal half-spread of 0.3 pips, and adverse selection values of $$\alpha \in \{0, 0.05, 0.15, 0.3\}$$ pips (representing no adverse selection, corporate hedger, mixed flow, and aggressive systematic flow respectively). We simulate 100,000 trade requests per parameter combination.

The simulation produces six key outputs.

**1. Fill rate surface.** Fill probability as a function of $$\tau$$ (x-axis) and $$\theta$$ (y-axis), rendered as a heatmap, with separate panels for asymmetric and symmetric designs. The iso-fill-rate contours are hyperbolas in $$(\tau, \theta)$$ space because fill rate depends only on $$\theta / \sigma\sqrt{\tau}$$. This immediately shows the substitutability of the two parameters: a dealer can achieve the same fill rate with a shorter window and tighter threshold, or a longer window and looser threshold.

![Fill Rate Surface](/assets/images/last-look/chart1_fill_rate_surface.png)

**2. Shadow spread vs. hold window.** The effective shadow spread as a function of $$\tau$$ for a few representative thresholds. Under asymmetric rejection with no adverse selection, the shadow spread grows monotonically with $$\tau$$. Under symmetric rejection with no adverse selection, the shadow spread is zero (a flat line). But under symmetric rejection *with* adverse selection ($$\alpha > 0$$), the shadow spread is positive and growing — this is the key chart that shows symmetric rejection still helps the dealer when flow is informed. Overlaying all three curves (asymmetric, symmetric with $$\alpha = 0$$, symmetric with $$\alpha > 0$$) makes the point better than any amount of algebra.

![Shadow Spread](/assets/images/last-look/chart2_shadow_spread.png)

**3. P&L distribution.** Violin plots showing the *conditional* (per-fill) P&L distribution for each design across adverse-selection tiers. Expectations lie; distributions don't.

![P&L Distribution](/assets/images/last-look/chart3_pnl_distribution.png)

**4. Optimal rejection threshold.** The threshold $$\theta^*$$ that maximises $$\mathbb{E}[\text{PnL per request}]$$ as a function of $$(\alpha, \tau)$$. This is what a desk actually calibrates to.

![Optimal Theta](/assets/images/last-look/chart4_optimal_theta.png)

**5. Per-request P&L vs adverse selection.** $$\mathbb{E}[\text{PnL per request}] = \text{fill rate} \times (s - \mathbb{E}[\Delta M \mid \text{fill}])$$ as a function of $$\alpha$$. Using per-request (throughput-penalised) P&L makes the symmetric/asymmetric difference starkly visible: asymmetric LL gains margin but kills fill rate for benign flow.

![PnL Per Request](/assets/images/last-look/chart6_pnl_per_request.png)

**6. PnL-per-request landscape.** $$\mathbb{E}[\text{PnL/req}]$$ as a function of $$(\theta, \alpha)$$ at fixed $$\tau = 75\text{ms}$$. The ridge of the optimal $$\theta^*$$ is overlaid. This directly answers: what does it cost to use the wrong threshold?

![PnL Landscape](/assets/images/last-look/chart7_pnl_landscape.png)

All six charts can be generated from the same Monte Carlo engine with different parameter sweeps and conditioning. The code is straightforward — the heavy lifting is in the truncated normal expectations, which have closed-form solutions, so the simulation mainly serves as a validation and a way to generate intuitive visuals. The accompanying Python notebook generates all six.
