...
Last Look didn't emerge from some grand market design. It was a patch. In the early 2000s, banks started streaming prices on ECNs and suddenly found themselves getting picked off by HFT firms with better technology. These firms were faster, saw price changes sooner, and systematically hit quotes that were already stale. The banks couldn't keep up technologically, so the ECNs gave them a workaround: a short window after receiving an order to check whether the price was still good. As Lehtis notes, it was introduced by one ECN specifically to level competitive advantages that certain clients possessed through superior technology[^15]. What started as a defensive tool for slow banks competing against fast HFTs gradually became embedded in the market's plumbing, and eventually got repurposed well beyond its original intent.
...
where the conditional expectation accounts for both the adverse selection drift and the truncation. Whether this exceeds $$s$$ depends on parameters. For uninformed flow ($$\alpha = 0$$), the truncation makes $$\mathbb{E}[\Delta M \mid \Delta M \leq \theta]$$ negative, so the client pays more than the quoted spread. The excess is a **shadow spread**: a hidden cost arising from asymmetric truncation. For informed flow ($$\alpha > 0$$), the adverse selection drift pushes the conditional expectation back toward positive territory, and for sufficiently toxic flow the client can still pay less than $$s$$ per fill even under asymmetric rejection, because their information advantage outweighs the truncation cost. But the client also bears the cost of *rejected* trades: re-entering the market at a worse price, or missing the opportunity entirely. The total effective cost must account for both the per-fill cost and the opportunity cost of rejects. Cartea, Jaimungal & Walton give the formal treatment[^10].

Under the symmetric design with adverse selection, the client's per-fill cost is $$s - \mathbb{E}[\Delta M \mid \lvert\Delta M\rvert \leq \theta]$$, which is strictly *less than* $$s$$ whenever $$\alpha > 0$$. There is no shadow spread above the quoted spread. What symmetric rejection does is reduce the client's information rent: without Last Look the informed client captures $$\alpha$$ per trade, but with symmetric rejection they capture only $$\mathbb{E}[\Delta M \mid \lvert\Delta M\rvert \leq \theta] < \alpha$$. The client still benefits from being informed, just less so. This is a qualitatively different story from asymmetric rejection on uninformed flow, where the client genuinely pays more than the quoted spread.
...
A few things jump out. First, the dashed black line (no Last Look) is a simple declining line: $$s - \alpha$$. No rejection, no fill rate penalty, but you eat the full adverse selection. Second, for benign flow (low $$\alpha$$), asymmetric Last Look actually *underperforms* no Last Look on a per-request basis at these parameters, because the gain per fill doesn't compensate for the volume you're throwing away. This result is not universal: it depends on the ratio $$s / \sigma\sqrt{\tau}$$. With a wide spread (large $$s$$), the dealer already has a thick cushion, so the truncation benefit is small relative to the volume cost of rejection. With tight spreads, the truncation bonus can outweigh the throughput penalty even on uninformed flow. The crossover point where asymmetric LL starts to beat no LL shifts with the spread. Third, the shaded region between the no-LL line and the symmetric LL curves represents the net benefit of symmetric rejection. It grows with $$\alpha$$, confirming what we saw in the shadow spread chart: symmetric rejection is most valuable precisely when adverse selection is highest. And fourth, the threshold calibration matters. At $$\theta$$ = 0.05 pips (most aggressive), you're rejecting almost everything. At $$\theta$$ = 0.20 pips (loosest), you barely reject at all. The right threshold depends on your flow composition, which changes through the day and across client tiers.
...
The intellectual arc here is worth stepping back to appreciate. A practice that started as informal courtesy in voice-brokered FX ("let me check the market before I confirm") became encoded into electronic trading platforms, got systematically exploited, drew regulatory sanction, prompted a global code of conduct, and is now the subject of a rich academic literature connecting it to optimal control theory, option pricing, and mechanism design. Meanwhile, the original rationale is eroding. The infrastructure gap between banks and non-bank LPs has narrowed to the point where the "we need protection from faster participants" argument carries less weight than it did in 2005. Some platforms are trying structural solutions instead: ParFX introduced a randomised pause of 20 to 80 milliseconds on all order activity, eliminating speed advantages entirely[^16]. Yet fill rates on major venues have actually been *declining*: Euronext FX dropped from 83.9% in 2023 to 74.7% in H2 2024, and Cboe FX slipped from 91% to 87% over the same period[^17], partly driven by new non-bank LPs entering FX with more aggressive rejection. The irony is that as technology closes the gap that originally justified Last Look, the practice may be getting more aggressive, not less. But Last Look is not some immutable feature of FX markets. It's a design choice, and people are actively working on alternatives. ParFX attacked the speed problem. OneChronos, which has already handled $2.85 trillion in US equities through its combinatorial auction ATS, recently launched a spot FX venue that sidesteps the entire bilateral model: blind auctions every 100 milliseconds, no FIFO ordering, no streaming quotes, no conditional veto[^18]. Orders are matched through an optimization algorithm that maximises fill quality across multiple currency pairs simultaneously. It's the most radical structural departure from the model that produced Last Look in the first place, and whether or not it gains traction, it's proof that the market design question is genuinely open. The bilateral streaming model with Last Look bolted on top is one answer. It doesn't have to be the only one.
...
[^15]: Lehtis, E. (2018). "Follow-Up on the Winners Curse (and a First Look at Last Look)." [LinkedIn](https://www.linkedin.com/pulse/follow-up-winners-curse-first-look-last-erik-lehtis).

[^16]: ParFX (2013). "Hardwiring best practice into spot FX trading venues." [ParFX](https://www.parfx.com/news/press-releases/hardwiring-best-practice-into-spot-fx-trading-venues/). ParFX was launched by Tradition with 14 founding bank members including Citi and JPMorgan.

[^17]: Lambert, C. (2025). "FX Trading Venues in 2024: A Big Tick." *The Full FX*. [Full article](https://thefullfx.com/fx-trading-venues-in-2024-a-big-tick/). Fill rate data from Euronext FX and Cboe FX platform reporting.

[^18]: Lambert, C. (2026). "OneChronos Launches Auction-Based FX Platform." *The Full FX*. [Full article](https://thefullfx.com/onechronos-launches-auction-based-fx-platform/). OneChronos has operated a US equities ATS since 2022, facilitating approximately $2.85 trillion in institutional transactions.
