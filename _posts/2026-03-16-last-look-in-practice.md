...
# Where Last Look Is Heading

The original rationale for Last Look is eroding. The infrastructure gap between banks and non-bank LPs has narrowed considerably since 2005, and the "we need protection from faster participants" argument carries less weight when your own stack can process a price check in single-digit milliseconds. Some platforms are addressing the latency problem structurally rather than giving one side a conditional veto. ParFX, launched in 2013 by a consortium of 14 banks including Citi and JPMorgan, introduced a randomised pause of 20 to 80 milliseconds on all order submissions, amendments, and cancellations, eliminating the advantage of speed-dependent strategies entirely[^16]. EBS has experimented with similar mechanisms.

Meanwhile, fill rates on major platforms have been declining. Euronext FX saw its platform fill rate drop from 83.9% in the first nine months of 2023 to 78.4% in 2024, with the sharpest decline in the second half: H1 averaged 82.2%, but H2 fell to just 74.7%[^17]. Cboe FX, which had been steady in the 89 to 91% range for over a year, slipped to 87 to 88% in the second half of 2024[^17]. Some of this is attributable to new non-bank LPs entering the FX space and running more aggressive rejection as they seek to replicate their success in equity and futures markets. The irony is that as technology closes the gap that originally justified Last Look, the practice may be getting more aggressive, not less.

Whether the market eventually converges on firm-quote models or simply tightens the parameters of the existing system remains an open question. But the direction of travel is clear: more transparency, more data, and fewer places to hide.
